...
Get 10-trail 5-fold results from the saved files. 
    ...
    @ method : name of method, can be base or betl
...
'''
def get_y_and_logits_results(dataset, backbone, method='base', if_get_logits=False, trails=10, folds=5):
    curr_dir = os.path.dirname(__file__)
    data_dir = os.path.join(curr_dir, '../outputs/result/', dataset, method, backbone)
    y_hat = []
    y_true = []
    logits = []
...
Calculate Gmean result of each trail
    ...
    @ folds : the number of folds results you want to get, default is 5
'''
def get_gmean_each_trial(dataset, backbone, method='base', trails=10, folds=5):
    curr_dir = os.path.dirname(__file__)
    data_dir = os.path.join(curr_dir, '../outputs/result', dataset, method, backbone)
    gmean = []
    for p in range(trails):
...
def draw_conf_matrix(class_list, y_true, y_hat, tail_idxes, pic_dir):
    ...
    fig.savefig(pic_dir, format='pNG', bbox_inches='tight')
...
'''
def draw_tail_classes_pr_curve(dataset, num_class, tail_idxes, pic_dir,trails):
    ...
    backbone = 'resnet18'
    method_list = [method]
    show_method_list = ['DTL', 'Base(Ours)']

    for i in range(len(method_list)):
        y_true, y_hat, logits = get_y_and_logits_results(dataset, backbone, method_list[i], if_get_logits=True,trails=trails)
...
Draw macro average Precision-Recall curves of all classes among different methods
    ...
    @ pic_dir : Precision-Recall curves picture save direction
'''
def draw_macro_pr_curve(dataset, num_class, method, pic_dir,trails):
...
def parse_args():
    ...
    parser.add_argument('--method', type=str, default='DDGF')
    ...
    parser.add_argument('--get_conf_matrix', type=str, default='True', help='If you want to get confusion matrix result, default is False.')
    parser.add_argument('--get_pr', type=str, default='True', help='If you want to get Precision-Recall curves result, default is False.')
    parser.add_argument('--get_macro_pr_all', type=str, default='True', help='If you want to get macro average Precision-Recall curves result of all classes, default is False.')
    parser.add_argument('--get_macro_pr_tail', type=str, default='True', help='If you want to get macro average Precision-Recall curves result of tail classes, default is False.')
...
if __name__ == '__main__':
    ...
    curr_dir = os.path.dirname(__file__)
    y_true, y_hat, logits = get_y_and_logits_results(dataset, backbone, method, if_get_logits=True,trails=trails)
    if dataset == 'KLSG':
        num_class = 2
        ...
        num_class = 8
        class_list = ['big_propeller', 'cylinder', 'fishing_net', 'floats',
                      'iron_pipeline', 'small_propeller', 'soft_pipeline', 'tire']
        tail_idxes = [0, 1, 2, 3, 4, 5, 6]
        colors = ['orangered', 'lightpink', 'coral', 'deeppink',
                  'lightseagreen', 'steelblue', 'magenta', 'cyan']
    else:
    ...
    if args.get_gmean in ['True', 'true']:
        ...
        gmean_dir = os.path.join(curr_dir, f'../outputs/display/gmean.txt')
        write_gmean(gmean, gmean_dir, dataset, method, backbone)
        # gmean_list = get_gmean_each_trial(dataset, backbone, method,trails=trails)
        # print(f'gmean list is : {gmean_list}')

    # write f1
    if args.get_f1 in ['True', 'true']:
        f1 = f1_score(y_true, y_hat, average='macro')
        f1_dir = os.path.join(curr_dir, f'../outputs/display/f1.txt')
        write_f1(f1, f1_dir, dataset, method, backbone)

    # save confusion matrix
    if args.get_conf_matrix in ['True', 'true']:
        save_folder = os.path.join(curr_dir, f'../outputs/display/cm')
        cm_dir = os.path.join(save_folder, f'{dataset}_{method}_{backbone}.png')
    ...
    if args.get_pr in ['True', 'true']:
        save_folder = os.path.join(curr_dir, f'../outputs/display/pr')
        pr_dir = os.path.join(save_folder, f'{dataset}_{method}_{backbone}.png')
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
        if 'svm' in method:
            probs = logits
    ...
    if args.get_macro_pr_all in ['True', 'true']:
        save_folder = os.path.join(curr_dir, f'../outputs/display/macro_pr')
        macro_pr_dir = os.path.join(save_folder, f'all_classes_{dataset}_{backbone}.png')
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
        draw_macro_pr_curve(dataset, num_class, method, macro_pr_dir,trails)

    # save tail-classes macro-pr curve
    if args.get_macro_pr_tail in ['True', 'true']:
        save_folder = os.path.join(curr_dir, f'../outputs/display/macro_pr')
        tail_macro_pr_dir = os.path.join(save_folder, f'tail_classes_{dataset}_{backbone}.png')
        if not os.path.exists(save_folder):
            os.makedirs(save_folder)
        draw_tail_classes_pr_curve(dataset, num_class, tail_idxes, tail_macro_pr_dir,trails)
